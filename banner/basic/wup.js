function getImages(div){
	...
	jQuery(divImage).find("img").each(function(){
		if(count == 1){
			jQuery(".pagCount").append("<div class='contadorImagens ativado imageNumber_"+count+"'>"+count+"</div>");
		}
		else{
			jQuery(".pagCount").append("<div class='contadorImagens desativado imageNumber_"+count+"'>"+count+"</div>");
		}
		count++;
	...
	jQuery("#container").mouseover(function(){
		checkEventOver = true;
		jQuery("#controlRight").animate({opacity:0.9});
		jQuery("#controlLeft").animate({opacity:0.9});
	});
	...
	jQuery("#container").mouseout(function(){
		if(jQuery(this).is(":hover")){
			...
			jQuery("#controlRight").animate({opacity:0});
			jQuery("#controlLeft").animate({opacity:0});
			checkEventOver = false;
			setTimeout(function(){ autoPass(); },5000);
		}
	});

	function autoPass(){
		setTimeout(function(){
			if(checkEventOver == false){
				jQuery("#controlRight").trigger("click");
				autoPass();
			}
		},5000);
